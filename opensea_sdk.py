...
class OpenseaStreamClient():
    def __init__(self, api_key: str, network: Network = Network.MAINNET):
        ...
        self.socket = Socket(socketUrl, hb_interval=0.01)
    ...
    def onCollectionOffer(self, collection_slug: str, callback):
        try:
            Logger(f"Listening for collection offers on {collection_slug}").debug()
            return self.on(EventTypes.COLLECTION_OFFER, collection_slug, callback)
        except Exception as e:
            Logger(e).error()

    def onTraitOffer(self, collection_slug: str, callback):
        try:
            Logger(f"Listening for trait offers on {collection_slug}").debug()
            return self.on(EventTypes.TRAIT_OFFER, collection_slug, callback)
        except Exception as e:
            Logger(e).error()

    def onOrderInvalidate(self, collection_slug: str, callback):
        try:
            Logger(f"Listening for order invalidate on {collection_slug}").debug()
            return self.on(EventTypes.ORDER_INVALIDATE, collection_slug, callback)
        except Exception as e:
            Logger(e).error()

    def onOrderRevalidate(self, collection_slug: str, callback):
        try:
            Logger(f"Listening for order revalidate on {collection_slug}").debug()
            return self.on(EventTypes.ORDER_REVALIDATE, collection_slug, callback)
        except Exception as e:
            Logger(e).error()
